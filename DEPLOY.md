...
| **`Can't reach database server`** at **runtime** (API logs, e.g. forgot-password) | Neon **compute was cold / waking**, or Prisma kept a **dead connection**. Open **Neon** dashboard once to wake the branch. In Vercel, set **`DATABASE_URL`** to the **current** pooler string from Neon with **`connect_timeout=60`**, `sslmode=require`, `pgbouncer=true`; remove **`channel_binding=require`** if present. Forgot-password **retries** with **`prisma.$disconnect()`** between attempts. If **every** attempt still fails, the URL or Neon project is wrong/inactive — re-copy **both** `DATABASE_URL` and **`DIRECT_URL`** from Neon. **Vercel Pro:** `maxDuration` on this route is 60s so retries can finish; **Hobby** may time out at ~10s if Neon is very slow to wake. |
...
| **`Resend 403: You can only send testing emails to your own email address (...)`** | Resend is working; **`RESEND_API_KEY`** is valid. Default **`from`** (`onboarding@resend.dev`) may **only** send to the email address stated in the error (your Resend account email). To send resets to **any** user: add & verify your domain at **[resend.com/domains](https://resend.com/domains)** (DNS records), then set **`RESEND_FROM_EMAIL`** in Vercel to something like **`noreply@yourdomain.com`**, redeploy, and try again. |
