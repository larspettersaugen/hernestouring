...
| **Forgot password: “Something went wrong”** (red error) | Usually **500/503** from the API. Open **Vercel → Logs** and look for **`[forgot-password] Request failed:`** — often **`PasswordResetToken` / relation does not exist** → run migrations on production (`prisma migrate deploy` is in the Vercel build; confirm **`DIRECT_URL`** and that deploy succeeded). |
